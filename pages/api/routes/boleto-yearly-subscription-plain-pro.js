  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res
        ...
        .json({ error: data || "Erro desconhecido da API Asaas" });
    }

    // Verifique se o URL do boleto está presente na resposta
    if (data && data.data && data.data[0] && data.data[0].bankSlipUrl) {
      const novoBoleto = new Boleto({
        userId: userId,
        bankSlipUrl: data.data[0].bankSlipUrl,
        subscriptionId: id,
      });
