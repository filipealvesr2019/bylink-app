...
export default function Payment() {
  ...
  const handleQRcode = async () => {
    ...
    try {
      const response = await axios.post(
        "/api/routes/qrcode-yearly-subscription-plain-pro"
      );
  ...
  const handleMonthlySubscriptionPix = async () => {
    ...
    try {
      const response = await axios.post(
        "/api/routes/yearly-subscription-pix-plain-pro"
      );
      if (response.status === 201) {
  ...
  const handleBoletoPayment = async () => {
    ...
    try {
      const response = await axios.get(
        "/api/routes/boleto-yearly-subscription-plain-pro"
      );
      if (response.status === 201) {
  ...
  const handleMonthlySubscriptionBoleto = async () => {
    ...
    try {
      const response = await axios.post(
        "/api/routes/yearly-subscription-boleto-plain-pro"
      );
      if (response.status === 201) {
        handleBoletoPayment();
      }
  ...
  const handleMonthlySubscriptionCartaoDeCredito = async () => {
    const result = await handleRequest("/api/routes/yearly-subscription-cartao-de-credito-plain-pro", "POST", {
      holderName,
      number,
      expiryMonth,
      expiryYear,
